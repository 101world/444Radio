...
export default function QuestsPage() {
  ...
  const filteredQuests = quests.filter(q => {
    ...
    // In development mode, show all quests regardless of level
          ...
          <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[200] animate-[slideDown_0.4s_cubic-bezier(0.22,1,0.36,1)]">
            <div className="flex items-center gap-3 pl-4 pr-5 py-3.5 rounded-2xl border border-cyan-500/40 shadow-2xl shadow-cyan-500/20"
              ...
              <div>
                ...
                <p className="text-xs text-cyan-300/80">{claimToast.title} — <span className="text-cyan-400 font-bold">+{claimToast.credits} credits</span></p>
              </div>
              <button onClick={() => setClaimToast(null)} className="ml-2 text-cyan-400/40 hover:text-white transition-colors">✕</button>
            </div>
          </div>
        )}
