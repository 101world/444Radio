...
export const ORGANIZED_METHODS: MethodSection[] = [
  // ══════ UNIVERSAL — works on any channel ══════
  {
    label: 'Mix',
    icon: '📊',
    category: 'universal',
  ...
  // ══════ SOUND-SPECIFIC — affects tone & pitch ══════
  {
    label: 'Filter',
    icon: '🎛️',
    category: 'sound',
    ...
    <div className="flex-1 overflow-y-auto">
      ...
      <div className="flex items-center gap-1.5 px-2 pt-2 pb-1">
        <button
          ...
          style={{
            background: activeTab === 'sounds' ? '#2a2e34' : '#23262b',
            border: 'none',
        ...
        <button
          ...
          className="flex-1 text-[8px] font-bold uppercase tracking-[.15em] py-1.5 rounded-xl transition-all duration-[180ms] cursor-pointer"
          style={{
            background: activeTab === 'methods' ? '#2a2e34' : '#23262b',
            border: 'none',
      ...
      {/* ═══ SOUNDS tab — accordion categories grouped by type ═══ */}
      {activeTab === 'sounds' && (
        <div className="px-1 pb-3 space-y-0.5">
          ...
          {([
            ...
            { key: 'sound' as const, label: 'SOUNDS', icon: '🥁', color: '#b8a47f', desc: 'Drums, samples & FX' },
            { key: 'bank' as const, label: 'BANKS', icon: '🎛️', color: '#b86f6f', desc: 'Drum machine presets' },
            ...
            <div key={cat.label}>
              <button
                ...
                style={{
                  color: expandedCat === cat.label ? '#c8cdd2' : '#5a616b',
                  background: expandedCat === cat.label ? '#2a2e34' : 'transparent',
                }}
              >
                <span>{cat.icon}</span>
      ...
      {/* ═══ METHODS tab — knobs + buttons organized by category ═══ */}
      {activeTab === 'methods' && (
                      ...
                      <div className="flex flex-wrap gap-1 px-1.5 py-0.5">
                        {buttons.map(m => (
                          <button
                            ...
                            onMouseEnter={(e) => {
                              e.currentTarget.style.boxShadow = 'inset 2px 2px 4px #14161a, inset -2px -2px 4px #2c3036'
                            }}
                            onMouseLeave={(e) => {
                              e.currentTarget.style.boxShadow = '2px 2px 4px #14161a, -2px -2px 4px #2c3036'
                            }}
                          >
                            {m.code}
                          </button>
