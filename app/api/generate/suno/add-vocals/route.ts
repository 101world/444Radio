...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { uploadUrl, title, prompt, style, negativeTags: rawNegTags, model = 'V4_5PLUS', vocalGender } = body
    ...
    ;(async () => {
      try {
        ...
        console.log('🎤 [ADD-VOCALS] Raw poll response:', JSON.stringify(completed).substring(0, 2000))
        ...
        for (let i = 0; i < Math.min(2, tracks.length); i++) {
          ...
          const fileName = `vocals-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, '-')}-${i + 1}-${Date.now()}.wav`
          ...
          const musicRes = await fetch(`${supabaseUrl}/rest/v1/music_library`, {
            ...
            body: JSON.stringify({ clerk_user_id: userId, title: entryTitle, prompt: cleanPrompt, lyrics: entryLyrics, audio_url: r2.url, audio_format: 'wav', generation_params: { engine: '444-add-vocals', type: '444-add-vocals', model: validModel, source_url: uploadUrl, trackNumber: i + 1 }, status: 'ready' }),
          }).catch(() => {})
          ...
          try {
            const cmRes = await fetch(`${supabaseUrl}/rest/v1/combined_media`, {
              method: 'POST',
              headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
              body: JSON.stringify({ user_id: userId, type: 'audio', title: entryTitle, audio_prompt: cleanPrompt, lyrics: entryLyrics, audio_url: r2.url, is_public: false, genre: '444-add-vocals', metadata: JSON.stringify({ source: '444-add-vocals', model: validModel, duration: track.duration, style: cleanStyle || null, trackNumber: i + 1 }) }),
            })
        ...
        updateTransactionMedia({ userId, type: 'generation_music', mediaUrl: audioUrl || undefined, mediaType: 'audio', title: cleanTitle, extraMeta: { engine: '444-add-vocals' } }).catch(() => {})
