  ...
  try {
    ...
    ;(async () => {
      try {
        ...
        console.log('🎶 [VOICE-TO-MELODY] Raw poll response:', JSON.stringify(completed).substring(0, 2000))
        ...
        for (let i = 0; i < Math.min(2, tracks.length); i++) {
          ...
          const fileName = `v2m-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, '-')}-${i + 1}-${Date.now()}.wav`
          ...
          const musicRes = await fetch(`${supabaseUrl}/rest/v1/music_library`, {
            ...
            body: JSON.stringify({ clerk_user_id: userId, title: entryTitle, prompt: cleanTags, lyrics: entryLyrics, audio_url: r2.url, audio_format: 'wav', generation_params: { engine: '444-voice-to-melody', type: '444-voice-to-melody', model: validModel, source_url: uploadUrl, trackNumber: i + 1 }, status: 'ready' }),
          }).catch(() => {})
          ...
          try {
            const cmRes = await fetch(`${supabaseUrl}/rest/v1/combined_media`, {
              method: 'POST',
              headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
              body: JSON.stringify({ user_id: userId, type: 'audio', title: entryTitle, audio_prompt: cleanTags, lyrics: entryLyrics, audio_url: r2.url, is_public: false, genre: '444-voice-to-melody', metadata: JSON.stringify({ source: '444-voice-to-melody', model: validModel, duration: track.duration, tags: cleanTags || null, trackNumber: i + 1 }) }),
            })
