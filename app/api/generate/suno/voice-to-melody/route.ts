...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { uploadUrl, title, tags, negativeTags = '', model = 'V4_5PLUS', vocalGender } = body
    ...
    ;(async () => {
      try {
        ...
        const fileName = `v2m-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, '-')}-${Date.now()}.mp3`
        ...
        await fetch(`${supabaseUrl}/rest/v1/music_library`, {
          ...
          body: JSON.stringify({ clerk_user_id: userId, title: cleanTitle, prompt: cleanTags, lyrics: track.lyric || '', audio_url: r2.url, audio_format: 'mp3', generation_params: { engine: '444-voice-to-melody', model: validModel, source_url: uploadUrl }, status: 'ready' }),
        }).catch(() => {})
