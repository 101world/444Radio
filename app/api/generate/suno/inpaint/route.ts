...
const CREDIT_COST = SUNO_CREDIT_COSTS.inpaint

/**
 * POST /api/generate/suno/inpaint
 *
 * 444 Pro Inpaint — replace a section (6-60s) within a track.
 * Costs 11 credits. Returns NDJSON stream.
 *
 * Body: { taskId, audioId, prompt, tags, title, infillStartS, infillEndS, negativeTags? }
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { taskId, audioId, prompt, tags, title, infillStartS, infillEndS, negativeTags } = body

    // ---------- Validation ----------
    if (!taskId || !audioId) return NextResponse.json({ error: 'taskId and audioId are required' }, { status: 400 })
    if (typeof infillStartS !== 'number' || typeof infillEndS !== 'number') return NextResponse.json({ error: 'infillStartS and infillEndS (seconds) are required' }, { status: 400 })

    const duration = infillEndS - infillStartS
    if (duration < 6 || duration > 60) return NextResponse.json({ error: 'Replacement section must be between 6 and 60 seconds' }, { status: 400 })
    ...
    if (!deductRes.ok || !deductResult?.success) {
      await logCreditTransaction({ userId, amount: -CREDIT_COST, type: 'generation_music', status: 'failed', description: `444 Inpaint: ${cleanTitle}`, metadata: { audioId } })
      return NextResponse.json({ error: deductResult?.error_message || 'Failed to deduct credits' }, { status: 402 })
    }
    await logCreditTransaction({ userId, amount: -CREDIT_COST, balanceAfter: deductResult.new_credits, type: 'generation_music', description: `444 Inpaint: ${cleanTitle}`, metadata: { audioId, infillStartS, infillEndS, engine: '444-inpaint' } })
    ...
    ;(async () => {
      try {
        ...
        const taskRes = await replaceSection({
          taskId,
          audioId,
          prompt: cleanPrompt,
          tags: cleanTags,
        ...
        const newTaskId = taskRes.data.taskId
        await sendLine({ type: 'progress', message: 'Replacing section...', taskId: newTaskId })

        const completed = await pollTaskUntilDone(newTaskId)
        ...
        const fileName = `inpaint-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, '-')}-${Date.now()}.mp3`
        ...
        await fetch(`${supabaseUrl}/rest/v1/music_library`, {
          ...
          body: JSON.stringify({ clerk_user_id: userId, title: cleanTitle, prompt: cleanPrompt, lyrics: track.lyric || '', audio_url: r2.url, audio_format: 'mp3', generation_params: { engine: '444-inpaint', source_audio: audioId, infillStartS, infillEndS }, status: 'ready' }),
        }).catch(() => {})
        ...
        try {
          const cmRes = await fetch(`${supabaseUrl}/rest/v1/combined_media`, {
            method: 'POST',
            headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
            body: JSON.stringify({ user_id: userId, type: 'audio', title: cleanTitle, audio_prompt: cleanPrompt, lyrics: track.lyric || '', audio_url: r2.url, is_public: false, genre: cleanTags || null, metadata: JSON.stringify({ source: '444-inpaint', duration: track.duration }) }),
          })
        ...
        await refundCredits({ userId, amount: CREDIT_COST, type: 'generation_music', reason: `Inpaint error: ${String(error).substring(0, 80)}`, metadata: { audioId } })
