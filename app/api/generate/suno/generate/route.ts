...
const CREDIT_COST = SUNO_CREDIT_COSTS.generate // 2 credits (same as standard)
  ...
  try {
    ...
    ;(async () => {
      try {
        ...
        console.log('✅ R2 upload:', permanentAudioUrl)
        ...
        if (saveRes.ok) {
          ...
          console.log('✅ Saved to music_library:', savedMusic?.id)
        }
        ...
        try {
          const cmRes = await fetch(`${supabaseUrl}/rest/v1/combined_media`, {
            ...
            body: JSON.stringify({
              ...
              metadata: JSON.stringify({ source: 'pro-music', model, language, engine: '444-pro', duration: track.duration }),
            }),
          })
          if (cmRes.ok) {
          ...
          console.error('❌ combined_media save error:', e)
        }
        ...
        await sendLine({
          ...
          // Include second track URL if available (bonus)
          secondTrackUrl: tracks[1]?.audio_url || null,
        })
