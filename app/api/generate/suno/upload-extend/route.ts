  ...
  try {
    ...
    ;(async () => {
      try {
        ...
        const fileName = `upext-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, '-')}-${Date.now()}.mp3`
        ...
        await fetch(`${supabaseUrl}/rest/v1/music_library`, {
          ...
          body: JSON.stringify({ clerk_user_id: userId, title: cleanTitle, prompt: cleanPrompt, lyrics: track.lyric || '', audio_url: r2.url, audio_format: 'mp3', generation_params: { engine: '444-upload-extend', model, source_url: uploadUrl }, status: 'ready' }),
        }).catch(() => {})
        ...
        try {
          const cmRes = await fetch(`${supabaseUrl}/rest/v1/combined_media`, {
            method: 'POST',
            headers: { apikey: supabaseKey, Authorization: `Bearer ${supabaseKey}`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
            body: JSON.stringify({ user_id: userId, type: 'audio', title: cleanTitle, audio_prompt: cleanPrompt, lyrics: track.lyric || '', audio_url: r2.url, is_public: false, genre: cleanStyle || null, metadata: JSON.stringify({ source: '444-upload-extend', model, duration: track.duration }) }),
          })
