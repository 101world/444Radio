...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { uploadUrl, title, prompt, style, instrumental = false, model = 'V4_5ALL', vocalGender, negativeTags, personaId } = body
    ...
    ;(async () => {
      try {
        ...
        const fileName = `cover-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, '-')}-${Date.now()}.mp3`
        ...
        await fetch(`${supabaseUrl}/rest/v1/music_library`, {
          ...
          body: JSON.stringify({ clerk_user_id: userId, title: cleanTitle, prompt: cleanPrompt, lyrics: track.lyric || '', audio_url: r2.url, audio_format: 'mp3', generation_params: { engine: '444-cover', model, source_url: uploadUrl }, status: 'ready' }),
        }).catch(() => {})
