...
async function generateMusic01(userId: string, body: Record<string, unknown>, jobId: string, signal: AbortSignal) {
  ...
  await fetch(`${supabaseUrl}/rest/v1/music_library`, {
    ...
    body: JSON.stringify({
      ...
      generation_params: { model: '444-pro-music', voice_id, has_voice_ref: !!voice_file, has_instrumental_ref: !!instrumental_file, source: 'plugin' },
      status: 'ready',
    }),
  })
...
async function generateBeatmaker(userId: string, body: Record<string, unknown>, jobId: string) {
  ...
  await fetch(`${supabaseUrl}/rest/v1/music_library`, {
    ...
    body: JSON.stringify({
      ...
      audio_url: audioUrl,
      audio_format: 'wav',
      generation_params: { model: '444-beatmaker', duration, type: 'beatmaker', source: 'plugin' },
      genre: 'beatmaker',
      status: 'ready',
    }),
  })
