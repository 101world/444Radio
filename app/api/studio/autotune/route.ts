...
export async function POST(request: Request) {
  try {
    ...
    if (deductErr || !deductRow?.success) {
      await logCreditTransaction({ userId, amount: -1, type: 'generation_autotune', status: 'failed', description: 'Studio auto-tune: insufficient credits', metadata: { scale } })
      return corsResponse(
    ...
    await logCreditTransaction({
      userId, amount: -1, balanceAfter: deductRow.new_credits,
      type: 'generation_autotune',
      description: `Studio Auto-Tune (${scale})`,
      metadata: { scale, outputFormat },
    })
    ...
    while (result.status !== 'succeeded' && result.status !== 'failed') {
      if (Date.now() - startTime > 60000) {
        ...
        await logCreditTransaction({ userId, amount: 1, balanceAfter: (deductRow.new_credits || 0) + 1, type: 'credit_refund', description: 'Studio auto-tune timeout — refund', metadata: { scale } })
    ...
    if (result.status === 'failed') {
      ...
      await supabase
        ...
        .update({ credits: (deductRow.new_credits || 0) + 1 })
        .eq('clerk_user_id', userId);
      await logCreditTransaction({ userId, amount: 1, balanceAfter: (deductRow.new_credits || 0) + 1, type: 'credit_refund', description: `Studio auto-tune failed — refund`, metadata: { scale, error: String(result.error).substring(0, 200) } })
    ...
    if (!outputUrl) {
      ...
      await supabase
        ...
        .update({ credits: (deductRow.new_credits || 0) + 1 })
        .eq('clerk_user_id', userId);
      await logCreditTransaction({ userId, amount: 1, balanceAfter: (deductRow.new_credits || 0) + 1, type: 'credit_refund', description: 'Studio auto-tune no output — refund', metadata: { scale } })
