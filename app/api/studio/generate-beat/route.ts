...
export async function POST(request: Request) {
  ...
  try {
    ...
    await logCreditTransaction({
      userId, amount: -2, balanceAfter: row.new_credits,
      type: 'generation_music',
      description: `Studio Beat: ${prompt?.substring(0, 60)}`,
      metadata: { prompt: prompt?.slice(0, 100), duration, bpm, generation_type: 'generate-beat' },
    })
