...
export async function GET() {
  try {
    ...
    if (!res.ok) {
      console.error('Supabase error fetching extend items:', res.status)

      // Fallback: try music_library with JSONB filter
      const fallbackRes = await fetch(
        `${supabaseUrl}/rest/v1/music_library?clerk_user_id=eq.${userId}&generation_params=cs.${encodeURIComponent('{"type":"444-extend"}')}&order=created_at.desc&limit=200`,
        {
          headers: {
            apikey: supabaseKey,
            Authorization: `Bearer ${supabaseKey}`,
      ...
      if (fallbackRes.ok) {
        ...
        const tracks = (fallbackData || []).map((item: any) => ({
          id: item.id,
          title: item.title || 'Untitled Extended Track',
      ...
      return NextResponse.json({ error: 'Failed to fetch extend items' }, { status: 500 })
    }
    ...
    console.error('Error fetching extend items:', error)
