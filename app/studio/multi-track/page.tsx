...
function StudioContent() {
  ...
  // Handle Beat generation with queue system
  const handleBeatGenerated = useCallback(async (audioUrl: string, metadata: any) => {
    const queueId = `beat-${Date.now()}`;
    const trackName = `Beat - ${metadata.prompt.substring(0, 30)}...`;
    
    // Create new track for this generation
    const newTrackId = addEmptyTrack();
    ...
    const queueItem: QueueItem = {
      ...
      status: 'completed', // Already generated by modal
      result: { audioUrl, metadata },
      trackId: newTrackId,
      timestamp: Date.now(),
    };
  ...
  }, [addEmptyTrack, addClipToTrack, showNotification]);

  // Handle Song generation with queue system
  const handleSongGenerated = useCallback(async (audioUrl: string, metadata: any) => {
    const queueId = `song-${Date.now()}`;
    const trackName = `Song - ${metadata.title || metadata.prompt.substring(0, 30)}`;
    ...
    // Add clip to the new track
    addClipToTrack(newTrackId, audioUrl, trackName, 0);
    showNotification('Song generated and added to new track', 'success');
  ...
  }, [addEmptyTrack, addClipToTrack, showNotification, user]);
  ...
  // Execute stem split with selected format and queue integration
  const executeStemSplit = useCallback(async (format: 'mp3' | 'wav') => {
    if (!stemSplitClip) return;
    
    const queueId = `stems-${Date.now()}`;
    ...
    const queueItem: QueueItem = {
      ...
      status: 'processing',
      progress: 'Initializing stem separation...',
      timestamp: Date.now(),
    };
    ...
    try {
      ...
      setGenerationQueue(prev => prev.map(item => 
        item.id === queueId 
          ? { ...item, progress: 'Analyzing audio with AI...' }
          : item
      ));
      ...
      if (!data.success) {
        ...
        setGenerationQueue(prev => prev.map(item => 
          item.id === queueId 
            ? { ...item, status: 'failed', error: data.error || 'Stem splitting failed' }
            : item
        ));
      ...
      setGenerationQueue(prev => prev.map(item => 
        item.id === queueId 
          ? { ...item, progress: 'Creating stem tracks...' }
          : item
      ));
      ...
      if (stemEntries.length === 0) {
        setGenerationQueue(prev => prev.map(item => 
          item.id === queueId 
            ? { ...item, status: 'failed', error: 'No stems were generated' }
            : item
        ));
      ...
      for (const [stemName, stemUrl] of stemEntries) {
        ...
        const trackId = addTrack(trackName, stemUrl as string);
      ...
      try {
        ...
        for (const [stemName, stemUrl] of stemEntries) {
          const response = await fetch(`${supabaseUrl}/rest/v1/combined_media_library`, {
            ...
            body: JSON.stringify({
              clerk_user_id: user?.id,
              title: `${stemSplitClip.name} - ${stemName}`,
              audio_url: stemUrl,
              created_at: new Date().toISOString(),
            }),
          });
        ...
        console.error('Failed to save stems to library:', error);
      }
      ...
      setGenerationQueue(prev => prev.map(item => 
        item.id === queueId 
          ? { 
              ...
              status: 'completed', 
              result: { stems },
              trackId: createdTracks.join(', ')
            }
          : item
      ));
      
      showNotification(`✨ Split into ${stemEntries.length} stems`, 'success');
  ...
  }, [stemSplitClip, addTrack, showNotification, user]);
  ...
  return (
    ...
    >
      ...
      {/* Global drag overlay */}
      {isDragOver && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-30 flex items-center justify-center border-2 border-dashed border-cyan-500">
