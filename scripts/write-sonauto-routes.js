// Helper script to generate sonauto route files
const fs = require('fs')
const path = require('path')

// ─── EXTEND ROUTE ───
const extendRoute = String.raw`import { NextRequest, NextResponse } from 'next/server'
...
// Sonauto v2/extend on fal.ai costs $0.075/gen → 50% profit → 4 credits @ $0.035/credit
const CREDIT_COST = 4
const FAL_MODEL = 'sonauto/v2/extend'

async function runSonautoExtend(falKey: string, input: Record<string, unknown>): Promise<{ data: any }> {
  console.log('[Sonauto-Extend] Calling fal.ai:', FAL_MODEL)
  const res = await fetch(` + '`https://fal.run/${FAL_MODEL}`' + String.raw`, {
    method: 'POST',
    headers: { Authorization: ` + '`Key ${falKey}`' + String.raw`, 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  })
  if (!res.ok) {
    const errText = await res.text().catch(() => 'Unknown fal.ai error')
    throw new Error(` + '`Sonauto Extend failed (${res.status}): ${errText.substring(0, 200)}`' + String.raw`)
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const userRes = await fetch(
      ` + '`${supabaseUrl}/rest/v1/users?clerk_user_id=eq.${userId}&select=credits,free_credits`' + String.raw`,
      { headers: { apikey: supabaseKey, Authorization: ` + '`Bearer ${supabaseKey}`' + String.raw` } },
    )
    const users = await userRes.json()
    if (!users?.length) return NextResponse.json({ error: 'User not found' }, { status: 404 })
    const totalCredits = (users[0].credits || 0) + (users[0].free_credits || 0)
    if (totalCredits < CREDIT_COST) {
      return NextResponse.json({ error: ` + '`Insufficient credits. 444 Extend requires ${CREDIT_COST} credits.`' + String.raw`, creditsNeeded: CREDIT_COST, creditsAvailable: totalCredits }, { status: 402 })
    }

    const deductRes = await fetch(` + '`${supabaseUrl}/rest/v1/rpc/deduct_credits`' + String.raw`, {
      method: 'POST',
      headers: { apikey: supabaseKey, Authorization: ` + '`Bearer ${supabaseKey}`' + String.raw`, 'Content-Type': 'application/json' },
    ...
    if (!deductRes.ok || !deductResult?.success) {
      await logCreditTransaction({ userId, amount: -CREDIT_COST, type: 'generation_music', status: 'failed', description: ` + '`444 Extend: ${cleanTitle}`' + String.raw`, metadata: { audio_url } })
      return NextResponse.json({ error: deductResult?.error_message || 'Failed to deduct credits' }, { status: 402 })
    }
    await logCreditTransaction({ userId, amount: -CREDIT_COST, balanceAfter: deductResult.new_credits, type: 'generation_music', description: ` + '`444 Extend: ${cleanTitle}`' + String.raw`, metadata: { audio_url, side, engine: 'sonauto-extend' } })
    ...
    ;(async () => {
      try {
        await sendLine({ type: 'started', model: 'sonauto-extend' })
        ...
        const fileName = ` + '`extend-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, \'-\')}-${Date.now()}.wav`' + String.raw`
        const r2 = await downloadAndUploadToR2(audioFile.url, userId, 'music', fileName)
        if (!r2.success) throw new Error(` + '`Storage upload failed: ${r2.error}`' + String.raw`)

        await fetch(` + '`${supabaseUrl}/rest/v1/music_library`' + String.raw`, {
          method: 'POST',
          headers: { apikey: supabaseKey, Authorization: ` + '`Bearer ${supabaseKey}`' + String.raw`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
          body: JSON.stringify({ clerk_user_id: userId, title: cleanTitle, prompt: prompt || '', lyrics: result.data.lyrics || lyrics_prompt || '', audio_url: r2.url, audio_format: 'wav', generation_params: { engine: 'sonauto-extend', side, extend_duration: result.data.extend_duration, seed: result.data.seed }, status: 'ready' }),
        }).catch(() => {})

        let libraryId: string | null = null
        try {
          const cmRes = await fetch(` + '`${supabaseUrl}/rest/v1/combined_media`' + String.raw`, {
            method: 'POST',
            headers: { apikey: supabaseKey, Authorization: ` + '`Bearer ${supabaseKey}`' + String.raw`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
            body: JSON.stringify({ user_id: userId, type: 'audio', title: cleanTitle, audio_prompt: prompt || '', lyrics: result.data.lyrics || lyrics_prompt || '', audio_url: r2.url, is_public: false, genre: (Array.isArray(tags) ? tags.join(', ') : tags) || null, metadata: JSON.stringify({ source: 'sonauto-extend', side, extend_duration: result.data.extend_duration, seed: result.data.seed }) }),
          })
          if (cmRes.ok) { const d = await cmRes.json(); libraryId = (Array.isArray(d) ? d[0] : d)?.id }
        } catch {}

        updateTransactionMedia({ userId, type: 'generation_music', mediaUrl: r2.url, mediaType: 'audio', title: cleanTitle, extraMeta: { engine: 'sonauto-extend' } }).catch(() => {})
        notifyGenerationComplete(userId, libraryId || '', 'music', cleanTitle).catch(() => {})
        notifyCreditDeduct(userId, CREDIT_COST, ` + '`444 Extend: ${cleanTitle}`' + String.raw`).catch(() => {})
        ...
        await refundCredits({ userId, amount: CREDIT_COST, type: 'generation_music', reason: ` + '`Extend error: ${String(error).substring(0, 80)}`' + String.raw`, metadata: { audio_url } })
...
// ─── INPAINT ROUTE ───
const inpaintRoute = String.raw`import { NextRequest, NextResponse } from 'next/server'
import { auth } from '@clerk/nextjs/server'
import { downloadAndUploadToR2 } from '@/lib/storage'
import { logCreditTransaction, updateTransactionMedia } from '@/lib/credit-transactions'
import { refundCredits } from '@/lib/refund-credits'
...
async function runSonautoInpaint(falKey: string, input: Record<string, unknown>): Promise<{ data: any }> {
  console.log('[Sonauto-Inpaint] Calling fal.ai:', FAL_MODEL)
  const res = await fetch(` + '`https://fal.run/${FAL_MODEL}`' + String.raw`, {
    method: 'POST',
    headers: { Authorization: ` + '`Key ${falKey}`' + String.raw`, 'Content-Type': 'application/json' },
    body: JSON.stringify(input),
  ...
  if (!res.ok) {
    const errText = await res.text().catch(() => 'Unknown fal.ai error')
    throw new Error(` + '`Sonauto Inpaint failed (${res.status}): ${errText.substring(0, 200)}`' + String.raw`)
  }
  return { data: await res.json() }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!tags || (Array.isArray(tags) && tags.length === 0) || (typeof tags === 'string' && !tags.trim())) {
      return NextResponse.json({ error: 'Tags are required for inpaint' }, { status: 400 })
    }
    ...
    const totalCredits = (users[0].credits || 0) + (users[0].free_credits || 0)
    if (totalCredits < CREDIT_COST) {
      return NextResponse.json({ error: ` + '`Insufficient credits. 444 Inpaint requires ${CREDIT_COST} credits.`' + String.raw`, creditsNeeded: CREDIT_COST, creditsAvailable: totalCredits }, { status: 402 })
    }
    ...
    if (deductRes.ok) { const raw = await deductRes.json(); deductResult = Array.isArray(raw) ? raw[0] ?? null : raw }
    if (!deductRes.ok || !deductResult?.success) {
      await logCreditTransaction({ userId, amount: -CREDIT_COST, type: 'generation_music', status: 'failed', description: ` + '`444 Inpaint: ${cleanTitle}`' + String.raw`, metadata: { audio_url } })
      return NextResponse.json({ error: deductResult?.error_message || 'Failed to deduct credits' }, { status: 402 })
    }
    await logCreditTransaction({ userId, amount: -CREDIT_COST, balanceAfter: deductResult.new_credits, type: 'generation_music', description: ` + '`444 Inpaint: ${cleanTitle}`' + String.raw`, metadata: { audio_url, start, end, engine: 'sonauto-inpaint' } })
    ...
    ;(async () => {
      try {
        await sendLine({ type: 'started', model: 'sonauto-inpaint' })
        ...
        const fileName = ` + '`inpaint-${cleanTitle.substring(0, 30).replace(/[^a-zA-Z0-9]/g, \'-\')}-${Date.now()}.wav`' + String.raw`
        const r2 = await downloadAndUploadToR2(audioFile.url, userId, 'music', fileName)
        if (!r2.success) throw new Error(` + '`Storage upload failed: ${r2.error}`' + String.raw`)
        ...
        await fetch(` + '`${supabaseUrl}/rest/v1/music_library`' + String.raw`, {
          method: 'POST',
          headers: { apikey: supabaseKey, Authorization: ` + '`Bearer ${supabaseKey}`' + String.raw`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
          body: JSON.stringify({ clerk_user_id: userId, title: cleanTitle, prompt: parsedTags.join(', '), lyrics: lyrics_prompt || '', audio_url: r2.url, audio_format: 'wav', generation_params: { engine: 'sonauto-inpaint', start, end, tags: parsedTags, seed: result.data.seed }, status: 'ready' }),
        }).catch(() => {})
        ...
        try {
          const cmRes = await fetch(` + '`${supabaseUrl}/rest/v1/combined_media`' + String.raw`, {
            method: 'POST',
            headers: { apikey: supabaseKey, Authorization: ` + '`Bearer ${supabaseKey}`' + String.raw`, 'Content-Type': 'application/json', Prefer: 'return=representation' },
            body: JSON.stringify({ user_id: userId, type: 'audio', title: cleanTitle, audio_prompt: parsedTags.join(', '), lyrics: lyrics_prompt || '', audio_url: r2.url, is_public: false, genre: parsedTags.join(', ') || null, metadata: JSON.stringify({ source: 'sonauto-inpaint', start, end, seed: result.data.seed }) }),
          })
          if (cmRes.ok) { const d = await cmRes.json(); libraryId = (Array.isArray(d) ? d[0] : d)?.id }
        } catch {}

        updateTransactionMedia({ userId, type: 'generation_music', mediaUrl: r2.url, mediaType: 'audio', title: cleanTitle, extraMeta: { engine: 'sonauto-inpaint' } }).catch(() => {})
        notifyGenerationComplete(userId, libraryId || '', 'music', cleanTitle).catch(() => {})
        notifyCreditDeduct(userId, CREDIT_COST, ` + '`444 Inpaint: ${cleanTitle}`' + String.raw`).catch(() => {})
        ...
        await refundCredits({ userId, amount: CREDIT_COST, type: 'generation_music', reason: ` + '`Inpaint error: ${String(error).substring(0, 80)}`' + String.raw`, metadata: { audio_url } })
...
fs.writeFileSync(path.join(__dirname, 'app/api/generate/suno/extend/route.ts'), extendRoute)
fs.writeFileSync(path.join(__dirname, 'app/api/generate/suno/inpaint/route.ts'), inpaintRoute)
console.log('✅ Written extend + inpaint routes')
